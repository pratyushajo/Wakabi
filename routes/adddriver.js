...
router.post('/',function(req,res){
	...
	var mailing_address = req.body.mailing_address;
	...
	pg.connect(connectionString, function(err, client) {
        if (!err) {
          var queryString = "INSERT INTO drivers (license_number, name,phone_number,  mailing_address, number_of_rides_remaining, working, current_zone, has_trailer, rating, last_payment, total_rides_completed, time_last_ride,recommenders1 , recommenders2) VALUES ('"
            + license_number + "','" + driver_name + "','" + phone_number + "','" + mailing_address + "' ," + payment_option +  ",'true', '1', 'true', '100%', '" + moment().format('YYYY-MM-DD HH:mm:ssZ') + "', '0', '" + moment().format('YYYY-MM-DD HH:mm:ssZ') + "','" + recommenders1 + "','"  + recommenders2 + "')";
